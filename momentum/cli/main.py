...
def approve(
    ...
    """Approve an automation opportunity — gathers context, generates a plan via LLM, then activates."""
    from momentum.discovery.opportunity_engine import get_opportunity_by_id
    from momentum.discovery.workflow_builder import get_workflow_by_id
    from momentum.reporting.opportunity_formatter import format_approve_prompt
    from momentum.automation.replay_engine import replay_automation_against_history
    ...
    from momentum.agents.interpreter_agent import (
        generate_automation_plan,
        generate_automation_plan_offline,
        requires_api_key_for_plan,
        _get_api_key,
    )
    ...
    needs_key = requires_api_key_for_plan(workflow)

    if needs_key and not api_key:
        console.print()
        console.print(Panel.fit(
            "[bold red]API key required[/bold red]\n\n"
            "This workflow type ([bold]" + (user_context.get("workflow_type", "generic").replace("_", " ")) + "[/bold]) "
            "is not a known developer workflow.\n"
            "An OpenAI API key is required to generate a custom automation plan for it.\n\n"
            "[dim]Set it with: [bold]$env:OPENAI_API_KEY=sk-...[/bold]\n"
            "Or re-run [bold]python -m momentum approve " + opportunity_id + "[/bold] and enter it when prompted.[/dim]",
            border_style="red",
        ))
        raise typer.Exit(1)
    ...
    with Progress(SpinnerColumn(), TextColumn("{task.description}"), transient=True) as prog:
        task = prog.add_task(
            "Generating automation plan using LLM..." if api_key else "Building automation plan (offline)...",
            total=None,
        )
        if api_key:
            llm_plan = generate_automation_plan(workflow, user_context, api_key)
        if not llm_plan:
            llm_plan = generate_automation_plan_offline(workflow, user_context)

    console.print()
    generated_by = llm_plan.get("generated_by", "offline")
    model_label = f"[dim](Generated by: {llm_plan.get('model', 'offline')})[/dim]" if generated_by == "llm" else "[dim](Generated by: offline heuristic)[/dim]"
    console.print(model_label)
    console.print()
    ...
    permissions = list(set(p for s in steps for p in []))

    with Progress(SpinnerColumn(), TextColumn("{task.description}"), transient=True) as prog:
        task = prog.add_task("Running historical replay validation...", total=None)
        auto_temp = AutomationRecord(
            id=auto_id,
        ...
        replay_result = replay_automation_against_history(auto_temp, workflow)

    console.print(f"\n[cyan]Replay validation:[/cyan]")
    console.print(f"  Accuracy : {replay_result['accuracy']:.0%} over {replay_result['total_cases']} cases")
    console.print(f"  Threshold: {'[green]✓ Met[/green]' if replay_result['meets_threshold'] else '[yellow]⚠ Marginal[/yellow]'}")
    console.print(f"  Est. savings: {replay_result.get('estimated_weekly_hours_saved', 0):.1f} hours/week")
    ...
    with get_db() as db:
        ...
        if opp_record:
            ...
            opp_record.replay_results_json = json.dumps(replay_result)
